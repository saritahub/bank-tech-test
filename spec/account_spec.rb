...
RSpec.describe 'Account' do
  ...
  context 'Deposit method' do
    ...
    context 'Deposit amount raises error' do
      it 'Raises an error message if the deposit input value is not an integer' do
        account = Account.new
        expect { account.deposit('50') }.to raise_error('Please enter a numerical value above 0.01.')
      end

      it 'Second test: Raises an error message if the deposit input value is not an integer' do
        account = Account.new
        expect { account.deposit('String') }.to raise_error('Please enter a numerical value above 0.01.')
      end

      it 'Third test: Raises an error message if the deposit input value is not an integer' do
        account = Account.new
        expect { account.deposit(['Array']) }.to raise_error('Please enter a numerical value above 0.01.')
      end

      it 'Raises an error message if the deposit amount is negative' do
        account = Account.new
        expect { account.deposit(-1) }.to raise_error('Please enter a numerical value above 0.01.')
      end
    end
  end
  ...
  context 'Withdraw method with sufficient funds' do
    ...
    context 'Withdrawal amount raises error' do
      it 'Raises an error message if the withdrawal input value is not an integer' do
        account = Account.new
        account.deposit(100)
        expect { account.withdraw('50') }.to raise_error('Please enter a numerical value above 0.01 and less than your current balance: £100.')
      end

      it 'Second test: Raises an error message if the withdrawal input value is not an integer' do
        account = Account.new
        account.deposit(100)
        expect { account.withdraw('String') }.to raise_error('Please enter a numerical value above 0.01 and less than your current balance: £100.')
      end

      it 'Third test: Raises an error message if the withdrawal input value is not an integer' do
        account = Account.new
        account.deposit(100)
        expect { account.withdraw(['Array']) }.to raise_error('Please enter a numerical value above 0.01 and less than your current balance: £100.')
      end
    end
  end
  ...
  context 'Print statement' do
    it 'Print statement returns formatted transaction history as a hash in reverse order' do
      ...
      account.deposit(100)
      account.withdraw(75)
      expect(account.print_statement).to eq([{:balance=>"25", :credit=>"", :date=>Date.today.to_s, :debit=>"75"},
                                             {:balance=>"100", :credit=>"100", :date=>Date.today.to_s, :debit=>""}])
    end
    ...
    it 'Second test: Print statement returns formatted transaction history as a hash in reverse order' do
      ...
      account.deposit(500)
      account.withdraw(20)
      expect(account.print_statement).to eq([{:balance=>"480", :credit=>"", :date=>Date.today.to_s, :debit=>"20"},
                                              {:balance=>"500", :credit=>"500", :date=>Date.today.to_s, :debit=>""}])
    end

    it 'Raises an error message if the withdraw amount is negative' do
      account = Account.new
      account.deposit(100)
      expect { account.withdraw(-1) }.to raise_error('Please enter a numerical value above 0.01 and less than your current balance: £100.')
    end
